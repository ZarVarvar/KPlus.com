...
<body>
  ...
  $result = $PDO->PDO->query("SELECT * FROM `videocardss`");
...
<button class="add popup_link" href="#popupaddvideo">Добавить</button>
...
<div class="conteiner_tovar">
        ...
        <div class="tovar product" data-id="<?=$product['id']?>">
            ...
            <span class="tovar_text_har">
                (Объем видеопамяти: <span><?=$product['gddr']?></span>, Разрядность шины памяти: <span><?=$product['razryd']?></span>, Производитель: <span><?=$product['made']?></span>, Энергопотребление:
                <span><?=$product['volt']?></span>)
            </span>
            ...
            <a class="upravlenie" href="del_video.php?del_id= <?= $product['id'] ?>">Удалить</a>
