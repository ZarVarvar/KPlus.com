...
<body>
  ...
  $result = $PDO->PDO->query("SELECT * FROM `processors`");
...
<button class="add popup_link" href="#popupaddproc">Добавить</button>
...
<div class="conteiner_tovar">
        ...
        <div class="tovar product" data-id="<?=$product['id']?>">
            ...
            <span class="tovar_text_har">
                (Сокет: <span><?=$product['socket']?></span>, Количество ядер: <span><?=$product['count_yader']?></span>, Количество потоков:
                <span><?=$product['count_potok']?></span>, Производитель: <span><?=$product['made']?></span>, Кэш:
            ...
            <a class="upravlenie" href="del_proc.php?del_id= <?= $product['id'] ?>">Удалить</a>
